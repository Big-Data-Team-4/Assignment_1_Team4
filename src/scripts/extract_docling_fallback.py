#!/usr/bin/env python3
"""
Docling Image Detective - DVC Integration
Goal: Extract images and generate clean outputs with proper formatting
Updated for DVC pipeline with relative paths

Project Structure:
src/
├── scripts/
│   ├── extract_docling.py  (this file)
│   └── azuredoc.py         (Azure processor)
└── data/
    ├── raw/                (input PDFs)
    └── parsed/
...
# Import the Azure processor
try:
    from azuredoc import extract_document_content
    AZURE_AVAILABLE = True
    print("Azure processor imported successfully")
except ImportError as e:
    print(f"Azure processor not available: {e}")
...
class DoclingProcessor:
    """Clean Docling processor with Azure fallback using azuredoc.py."""
    
    def __init__(self, input_dir: str = "data/raw", output_dir: str = "data/parsed/fallback_output"):
    ...
    def count_markdown_image_placeholders(self, markdown_content: str) -> int:
        ...
        # Count both Docling and Azure style placeholders
        docling_count = markdown_content.count('<!-- image -->')
        azure_count = len(re.findall(r'!\[Figure', markdown_content))
        return max(docling_count, azure_count)
    
    def create_metadata_entry(self, pdf_path: Path, docling_images: Dict, actual_images: Dict, 
                            markdown_placeholders: int, processing_time: float,
                            processor: str = 'docling', fallback_used: bool = False,
                            error_message: str = None, tables_extracted: int = 0) -> Dict[str, Any]:
        ...
        metadata = {
            ...
            'images_detected': docling_images['total_images_detected'],
            'images_extracted': docling_images['images_extracted'],
            ...
            # Detailed results
            'extraction_details': docling_images['extraction_details'],
    ...
    def process_all_pdfs(self):
        ...
        # Show key statistics
        if successful > 0:
            total_docling_detected = sum(entry.get('images_detected', 0) for entry in self.metadata_entries if entry.get('processing_status') == 'success')
            total_docling_extracted = sum(entry.get('images_extracted', 0) for entry in self.metadata_entries if entry.get('processing_status') == 'success')
...
def main():
    ...
    parser.add_argument('--output', '-o', default='data/parsed/fallback_output', 
                       help='Output directory for results (default: data/parsed/fallback_output)')
